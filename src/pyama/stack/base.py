...
class provides basic stack functionality that may be used by subclasses.
class not intended for direct instantiation, but only for subclassing.

classes should provide this API:
...
class fires these event kinds with a dict as keyword argument `message`:
...
, all events contain the keyword argument `stack_id` holding the
of this stack, which can also be retrieved with the `id` property.
    """
...
method may be implemented/overwritten by a subclass.
a stack should always fire a `const.EVT_CLOSE` event
its listeners to allow for cleanup.
        """
...
are zero-based.
indices of existing dimensions are required.
is not supported.
        """
...
new shape must be passed as a dict-like object with preserved order.
